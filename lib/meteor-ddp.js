...
MeteorDdp.prototype.connect = function() {
  ...
  self.sock.onopen = function() {
    ...
    connectionTimeout = setTimeout(function() {
      self.sock.close();
      conn.reject(new Error('DDP connection timeout'));
    }, 2000);
  };

  //TODO investigate websocket timeout
  connectionTimeout = setTimeout(function() {
    self.sock.close();
    conn.reject(new Error('WebSocket connection timeout'));
  }, 2000);
  ...
  //reject if onclose before connected
...
MeteorDdp.prototype._resolveCall = function(data) {
  ...
  } else { // if (typeof data.result !== 'undefined') { //??? calls with no result
